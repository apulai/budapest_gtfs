...
def reconstruct_path(came_from, came_from_instruction,  current):
    total_path = [current]
    total_instruction = [came_from_instruction[current]]
    while current in came_from:
        a = came_from[current]
        current = came_from[current]
        instruction = came_from_instruction.get(a,"start")
        total_path.insert(0, current)
        total_instruction.insert(0, instruction)

    return total_path, total_instruction


def a_star(start, goal, h, get_neighbours, d):
    """
    ...
    Args:
        ...
        goal: Goal node
        h: Heuristic function h(n) that estimates cost from n to goal
        get_neighbours: Function that returns neighbors of a node
        d: Function d(current, neighbor) that returns edge weight
    ...
    while open_set:
        ...
        if current == goal:
            return reconstruct_path(came_from,came_from_instruction, current)

        neighbors, routing = get_neighbours(current)
        for idx,neighbor in enumerate(neighbors):
            ...
            if tentative_g_score < g_score[neighbor]:
                ...
                came_from_instruction[neighbor] = routing[idx]
...
for t in ltests:
    ...
    total_path, total_instruction = a_star(t["start_stop"],t["final_stop"], h, get_neighbours, h)

    print(f'{stops[t["start_stop"]]["stop_name"]} --> {stops[t["final_stop"]]["stop_name"]}')
    for (stop_id,instruction) in zip(total_path,total_instruction):
        if instruction.startswith("walk"):
            istr = f'{instruction:<6}   to  {stops[stop_id]["stop_name"]} {stop_id}'
        elif instruction == "start":
            istr = f'Start       at  {stops[stop_id]["stop_name"]} {stop_id}'
        else:
            istr = f'take {routes[instruction]["route_short_name"]:<6} to  {stops[stop_id]["stop_name"]:<30} headsign {routes[instruction]["route_desc"]:<70}  {stop_id}'
        print(istr)
